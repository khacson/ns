...
class OtherdebtModel extends CI_Model
{
	...
	function findID($id) {
        $query = $this->model->table('hre_otherdebt')
					  ->where('isdelete',0)
					  ->where('id',$id)
					  ->find();
        return $query;
	...
	function getSearch($search){
		...
		if(!empty($search['fromdate'])){
			$sql.= " and r.otherdebt_date >= '".fmDateSave($search['fromdate'])." 00:00:00' ";	
		}
		if(!empty($search['todate'])){
			$sql.= " and r.otherdebt_date <= '".fmDateSave($search['todate'])." 00:00:00' ";	
		}
		return $sql;
	}
	...
	function saves($array,$id){
		...
		$login = $this->login;
		 ...
		 $result = $this->model->table('hre_otherdebt')->insert($array);	
		 return $result;
	}
	function edits($array,$id){
		...
		$array['branchid'] = $login['branchid'];
		$array['otherdebt_money'] =  fmNumberSave($array['otherdebt_money']);
		$this->model->table('hre_otherdebt')
					->where('id',$id)
					->update($array);	
		return $id;
