...
class Applicationforleaveapproved extends CI_Controller {
	...
	function form(){
		...
		if(empty($find->id)){
			$find = $this->base_model->getColumns('hre_applicationforleave');
		}
	...
	function export(){
		...
		$sheetIndex->setCellValueByColumnAndRow(5, 1, getLanguage('bo-phan'));
	...
	function saveApproved(){
		...
		if($array['approved'] == 1){//Nếu cho nghỉ phép chấm vào bảng nghỉ phép
			...
			//Tong ngay phep
			$yearHoliday = $this->base_model->getCountHolidays($items); //Tong ngay nghi trong nam
			//Tinh hoi gian nghi
			$listDate =  $this->base_model->getListDay(fmDateTimeSave($array['time_start']),fmDateTimeSave($array['time_end']));
			print_r($array['time_end']); exit;
			//Tính ca làm việc tinh ra thoi gian
			$total = 0;
